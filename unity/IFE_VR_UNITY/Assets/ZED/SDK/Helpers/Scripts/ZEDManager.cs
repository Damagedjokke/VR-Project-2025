...
public class ZEDManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Improved XR device detection that works with OpenVR
    /// </summary>
    private bool HasXRDeviceImproved()
    {
        ...
        if (debug) Debug.Log($"HasXRDeviceImproved: original={originalCheck}, enabled={xrEnabled}, device={XRSettings.loadedDeviceName}, inputSubs={xrInputSubsystems.Count}");
...
#pragma warning restore 414
    ...
    private void CheckStereoMode()
    {
        ...
        if (debug) Debug.Log($"=== ZED STEREO MODE DEBUG ===");
        if (debug) Debug.Log($"Total cameras found: {cams.Length}");
        if (debug) Debug.Log($"allowARPassThrough: {allowARPassThrough}");

        List<Camera> monocams = new List<Camera>();
        foreach (Camera cam in cams)
        {
            if (debug) Debug.Log($"Camera: {cam.name}, StereoTargetEye: {cam.stereoTargetEye}");
            ...
            switch (cam.stereoTargetEye)
            {
                case StereoTargetEyeMask.Left:
                    if (!cameraLeft)
                    {
                        ...
                        if (debug) Debug.Log($"Left camera assigned: {cam.name}");
                    }
                    break;
                case StereoTargetEyeMask.Right:
                    if (!cameraRight)
                    {
                        ...
                        if (debug) Debug.Log($"Right camera assigned: {cam.name}");
                    }
                    break;
        ...
        if (debug) Debug.Log($"cameraLeft assigned: {cameraLeft != null}");
        if (debug) Debug.Log($"cameraRight assigned: {cameraRight != null}");

        // Check for ZEDRenderingPlanes
        ZEDRenderingPlane[] renderPlanes = GetComponentsInChildren<ZEDRenderingPlane>();
        if (debug) Debug.Log($"ZEDRenderingPlanes found: {renderPlanes.Length}");
        foreach (var plane in renderPlanes)
        {
            if (debug) Debug.Log($"  - RenderingPlane on {plane.gameObject.name}, viewSide: {plane.viewSide}");
        }
        ...
        {
            if (debug) Debug.Log("Searching monocams for ZEDRenderingPlane assignments...");
            foreach (Camera cam in monocams)
            {
                                   ...
                                   rendplane.viewSide == ZEDRenderingPlane.ZED_CAMERA_SIDE.LEFT_FORCE))
                {
                    cameraLeft = cam;
                    camLeftTransform = cam.transform;
                    if (debug) Debug.Log($"Left camera found via RenderingPlane: {cam.name}");
                }
                                         ...
                                         rendplane.viewSide == ZEDRenderingPlane.ZED_CAMERA_SIDE.RIGHT_FORCE))
                {
                    cameraRight = cam;
                    camRightTransform = cam.transform;
                    if (debug) Debug.Log($"Right camera found via RenderingPlane: {cam.name}");
                }
            }
        }

        // Now check if we have both cameras with correct stereo setup
        if (camLeftTransform && camRightTransform && cameraLeft.stereoTargetEye == StereoTargetEyeMask.Left)
        {
            if (debug) Debug.Log("Both cameras found with correct stereo target eyes");

            if (camLeftTransform.transform.parent != null)
            {
                zedRigRoot = camLeftTransform.parent;
                if (debug) Debug.Log($"zedRigRoot set to: {zedRigRoot.name}");
            }

            if (debug) Debug.Log($"hasXRDevice() returns: {ZEDSupportFunctions.hasXRDevice()}");
            if (debug) Debug.Log($"HasXRDeviceImproved() returns: {HasXRDeviceImproved()}");
            if (debug) Debug.Log($"XRSettings.enabled: {XRSettings.enabled}");
            if (debug) Debug.Log($"XRSettings.loadedDeviceName: {XRSettings.loadedDeviceName}");

            if (allowARPassThrough && HasXRDeviceImproved())
            {
                isStereoRig = true;
                Debug.Log("isStereoRig set to TRUE");

                List<XRInputSubsystem> subsystems = new List<XRInputSubsystem>();
                SubsystemManager.GetInstances<XRInputSubsystem>(subsystems);
                Debug.Log($"XR Input Subsystems found: {subsystems.Count}");

                for (int i = 0; i < subsystems.Count; i++)
                {
                    subsystems[i].TrySetTrackingOriginMode(TrackingOriginModeFlags.Device);
                    subsystems[i].TryRecenter();
                    Debug.Log($"  - Subsystem {i} configured");
                }
            }
            else
            {
                isStereoRig = false;
                Debug.Log("✗ allowARPassThrough is false, stereo rig disabled");
        ...
        {
            Debug.LogWarning("✗ Stereo rig requirements NOT met:");
            Debug.LogWarning($"  - camLeftTransform exists: {camLeftTransform != null}");
            Debug.LogWarning($"  - camRightTransform exists: {camRightTransform != null}");
            if (cameraLeft)
                Debug.LogWarning($"  - cameraLeft.stereoTargetEye: {cameraLeft.stereoTargetEye}");
        ...
        Debug.Log($"=== FINAL: isStereoRig = {isStereoRig} ===");
    }
    #endregion
    ...
    void Awake()
    {
        ...
        Debug.Log("=== ZEDManager Awake Started ===");
        Debug.Log($"GameObject name: {gameObject.name}");
        Debug.Log($"allowARPassThrough setting: {allowARPassThrough}");
        ...
        if (isStereoRig && HasXRDeviceImproved())
        {
    ...
    private void ZEDReady()
    {
        ...
        Debug.Log("=== ZEDReady() START ===");
        Debug.Log($"isStereoRig: {isStereoRig}");
        Debug.Log($"hasXRDevice: {ZEDSupportFunctions.hasXRDevice()}");
        ...
        if (enableTracking)
        {
            Debug.Log("Waiting for tracking thread...");
            trackerThread.Join();
            Debug.Log("Tracking thread finished");
        }

        // Add safety checks to prevent crash
        if (isStereoRig && ZEDSupportFunctions.hasXRDevice())
        {
            Debug.Log("=== ATTEMPTING AR PASSTHROUGH INITIALIZATION ===");
            Debug.Log($"arRig exists: {arRig != null}");
            Debug.Log($"zedCamera.IsCameraReady: {zedCamera != null && zedCamera.IsCameraReady}");

            // Search for existing calibration files (for informational purposes)
            Debug.Log("=== SEARCHING FOR CALIBRATION FILES ===");
            string[] possiblePaths = new string[]
            {
        Application.dataPath + "/CalibrationZEDHMD.ini",
        Application.dataPath + "/../CalibrationZEDHMD.ini",
        "C:/ProgramData/stereolabs/settings/CalibrationZEDHMD.ini",
        System.Environment.GetFolderPath(System.Environment.SpecialFolder.ApplicationData) + "/stereolabs/settings/CalibrationZEDHMD.ini",
            };

            bool foundCalibFile = false;
            foreach (string path in possiblePaths)
            {
                if (System.IO.File.Exists(path))
                {
                    Debug.Log($"✓ FOUND calibration file at: {path}");
                    foundCalibFile = true;
                }
                else
                {
            ...
            if (!foundCalibFile)
            {
                Debug.LogWarning("WARNING: No calibration file found. AR passthrough may not work correctly.");
                Debug.LogWarning("Please run the ZED Calibration Tool to create CalibrationZEDHMD.ini");
            }

            // Only proceed if everything is ready
            if (arRig != null && zedCamera != null && zedCamera.IsCameraReady)
            {
                try
                {
                    Debug.Log("Calling arRig.InitTrackingAR()...");
                    ZEDMixedRealityPlugin.Pose pose = arRig.InitTrackingAR();
                    OriginPosition = pose.translation;
                    OriginRotation = pose.rotation;
                    Debug.Log($"✓✓✓ AR Tracking initialized successfully!");
                    Debug.Log($"  Position: {OriginPosition}");
                    Debug.Log($"  Rotation: {OriginRotation}");

                    if (!zedCamera.IsHmdCompatible && zedCamera.IsCameraReady)
                        Debug.LogWarning("WARNING: AR Passthrough with a ZED (not ZED Mini/2) is not recommended.");
                }
                catch (System.Exception e)
                {
                    Debug.LogError($"EXCEPTION during InitTrackingAR: {e.Message}");
                    Debug.LogError($"Stack: {e.StackTrace}");
                    Debug.LogError("AR passthrough failed to initialize. Make sure you've run the ZED Calibration Tool.");
            ...
            {
                Debug.LogWarning("Cannot initialize AR passthrough - requirements not met:");
                Debug.LogWarning($"  arRig null: {arRig == null}");
                Debug.LogWarning($"  zedCamera null: {zedCamera == null}");
                Debug.LogWarning($"  zedCamera ready: {zedCamera != null && zedCamera.IsCameraReady}");
        ...
        {
            Debug.Log($"NOT initializing AR passthrough. isStereoRig: {isStereoRig}, hasXRDevice: {ZEDSupportFunctions.hasXRDevice()}");
            OriginPosition = initialPosition;
            OriginRotation = initialRotation;
        }

        Debug.Log($"Final OriginPosition: {OriginPosition}");
        Debug.Log($"Final OriginRotation: {OriginRotation}");

        //Set the original transform for the Rig
        zedRigRoot.localPosition = OriginPosition;
        zedRigRoot.localRotation = OriginRotation;

        Debug.Log("=== ZEDReady() END ===");
    }
